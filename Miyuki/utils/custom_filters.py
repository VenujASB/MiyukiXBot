...
from Miyuki import BOT_ID
from Miyuki.mongo.disabledb import DISABLED_CMDS
from Miyuki.core.caching import ADMIN_CACHE, admin_cache_reload
...
async def bot_admin_check_func(_, __, m: Message or CallbackQuery):
    ...
    if BOT_ID in admin_group:
        return True
    await m.reply_text(
        "I don't have enough permissions, Mind Promoting?",
    )
    return False
