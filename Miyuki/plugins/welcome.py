...
from Miyuki import *
from Miyuki.mongo.gban import GBan
from Miyuki.mongo.welcomedb import Greetings
from Miyuki.utils.custom_filters import admin_filter, command
from Miyuki.utils.string import (
    build_keyboard,
    parse_button,
)
...
my news channel @TeamMiyukiBot.

**New to Me, Touch the below button and start me in PM**
                    """,
                    reply_markup=InlineKeyboardMarkup(
            [
                InlineKeyboardButton("quick start guide", url="http://t.me/TheMiyukiXBot?start=help"),
            ]))
        ...
        if member.id == OWNER_ID:
               ...
               return
        if member.id == 1984415770:#for @venujast 
               await app.send_message(
                message.chat.id,
...
async def member_has_left(_, message: Message):
    ...
    for member in message.left_chat_member:
        try:
            if member.id == 1984415770:
               await app.send_message(
                message.chat.id,
                "My Developer [Venuja Sadew](https://t.me/VenujaST) was left.",
            )
               return
            if member.id == BOT_ID:
                await app.send_message(
                chat_id=LOG_GROUP_ID,
                text=f"""
Owner of {chat_title} was removed me from there !
                """
            )
                return await remove_served_chat(message.chat.id) 
            if member.is_bot:
               botname = member.username
