...
from Miyuki import app,BOT_ID
from Miyuki.utils.filter_groups import cbot
from lang import get_command
from Miyuki.utils.lang import language
from Miyuki.mongo import chatb
from Miyuki.plugins.antlangs import get_arg
from Miyuki.utils.custom_filters import admin_filter
...
async def szcbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Miyuki&ownername=@venujast&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Miyuki")
           if "Lebyy_Dev" in textmsg:
               textmsg = textmsg.replace("Lebyy_Dev", "Venuja Sadew")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=Miyuki&bot_master=Venuja&user_id={user_id}")
           textmsg = (safeone.json()["answer"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Miyuki")
           if "[Safone]" in textmsg:
               textmsg = textmsg.replace("[Safone]", "Venuja Sadew")
           msg = tr.translate(textmsg, src='en', dest=lang)
           await message.reply_text(msg.text)
